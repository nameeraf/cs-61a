...
(define (zip pairs)
  (if (null? pairs) ((list pairs pairs))
...
(define (enumerate s)
  ; BEGIN PROBLEM 15
  (define (indexfinder s index)
    (cond ((null? s) s)
          (else (cons (list index (car s)) (indexfinder (cdr s) (+ index 1))))
    )
  )
  (indexfinder s 0)
)
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           ; BEGIN PROBLEM EC
           (cons (cons 'lambda (cons (car (zip (map let-to-lambda values))) (map let-to-lambda body))) (car (cdr (zip (map let-to-lambda values)))))
           ; END PROBLEM EC
           ))
        (else
         ; BEGIN PROBLEM EC
